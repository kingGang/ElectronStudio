...
// 俯仰 -20~180、身体 -90~90。
//
// 两臂俯仰上限都收到 150。固件那张模型角↔舵机角映射表标着 "Need to adjust parameters for specific
// hardware"，精英版的实际约束比它假设的紧，真机实测两条：
//   - 167° 顶到机械限位堵转 → 电流尖峰 → 舵机不 ACK → 固件 I²C 无限重试自旋 → 主控硬死（需断电）。
//   - 160° 手会撞到头（哪怕横滚已经张到最大 30°）。
// 取 150 同时避开这两条。宁可少几十度，也不能让上层（滑杆/动作编排/大模型调的动作）命令得到那里。
//
// 【还有一条限位管不住的坑】：胳膊【高举时把横滚收回来】同样会撞头（手往脑袋上招）。这条只能在
// 编排里避免——见 choreography/builtins.go：pitch 高时 roll 不得低于 ~18。
var JointLimits = [JointCount][2]float32{{-15, 15}, {0, 30}, {-20, 150}, {0, 30}, {-20, 150}, {-90, 90}}
